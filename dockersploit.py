...
def read_stats(id):
    ...
    return cpu_percent, mem_percent, ", ".join(ids), ", ".join(names), mem_usages
...
def cve(args):
    ...
    if is_dockerfile(dockerfile_path):
        ...
        if "privesc" in servicesPort.keys():
            ...
            print("The container can be accessed through these ports:")
    ...
    if os.name == "posix":
        if p != "" and port_list != "":
            ...
            for a in port_list:
                ...
                #l.append(str(p[a][0]['HostPort']) + " -> " + str(a) + " " + str(servicesPort[b]))
