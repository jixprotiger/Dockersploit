...
def run_docker():
   ...
   return redirect("http://localhost:5000/details?cve_number="+cve_number, code=302)
...
def Delete():
   cve_number = request.args.get("cve_number")
   delete = "true"
   #lines,a  = dockersploit.read_running()
   #running = []
   #for line in lines:
   #     running.append(line.strip())
   dockersploit.delete(str(cve_number))
   return redirect("http://localhost:5000/?delete=true&cve_number="+cve_number, code=302)
...
def Details():
    ...
    if cve_number in ports.keys():
        ...
        cpu, mem, docker_id, docker_name, mem_usage = dockersploit.read_stats(cve_number)
        cpu_format = "{:.2f}".format(cpu)
        return render_template("details.html", c = c, cve_number = cve_number, running = running, port_list = port_list, cpu = cpu_format, mem = mem, docker_id = docker_id, docker_name = docker_name, started=True, mem_usage = "{:.2f}".format((mem_usage*4/8/1048576)))
    else:
...
def Usage():
    ...
    cpu,ram,c_id,name, mem_usage = dockersploit.read_stats(cve_number)
    cpu_format = "{:.2f}".format(cpu)
    mem = "{:.2f}".format((mem_usage*4/8/1048576))
    return str(cpu_format) + "," + str(ram/2) + "," + str(mem)
